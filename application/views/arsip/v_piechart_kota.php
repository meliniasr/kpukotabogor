        ...
        <div class="card">
          ...
          <div class="card-body">
            <h4 style="font-family:'Franklin Gothic Medium', 'Arial Narrow', Arial, sans-serif; text-align:center;">
              PEROLEHAN SUARA SAH PASANGAN CALON PRESIDEN DAN WAKIL PRESIDEN <br> TAHUN <?= $detail->tahun?> DI KOTA
              BOGOR
            </h4>
            <br>
          ...
          <div class="card-body">
            ...
            <table id="example1" class="table table-bordered table-striped">
              ...
              <tbody>
                <?php
                      $no = 1;
                      foreach ($piechartkota as $detail) { ?>
                <tr id="tabel">
                  <td>1</td>
                  <td colspan="2"><?= $detail->nama_capres_satu?></td>
                  <td><?= number_format((float)$detail->rincian_kotabogor_satu)?></td>
                  <td><?= number_format((float)$detail->rincian_persen_kotabogor_satu, 1 )?>%</td>
                  <td rowspan="3">
                    <a href="<?= base_url('admin/perolehan_suarasah_kota/edit/' . $detail->id) ?>"
                      class="btn btn-primary btn-sm"><i class="fa fa-edit"></i></a>
                    <a onclick="return confirm('Anda yakin ingin menghapusnya?');"
                      href="<?= base_url('admin/perolehan_suarasah_kota/hapus/' . $detail->id) ?>"
                ...
                <tr>
                  ...
                  <td><?= number_format((float)$detail->rincian_kotabogor_dua)?></td>
                  <td><?= number_format((float)$detail->rincian_persen_kotabogor_dua, 1)?>%</td>
                </tr>
                <?php } ?>
              </tbody>
            </table>
            ...
            Highcharts.chart('piechart_kota', {
              ...
              series: [{
                ...
                data: [
                  <?php
									      $no = 1;
					if(is_array($piechartkota)){
						foreach ($piechartkota as $key => $detail) {
							  $nama_capres_satu = $detail->nama_capres_satu;
								$nama_capres_dua = $detail->nama_capres_dua;
								$rincian_kotabogor_satu = $detail->rincian_kotabogor_satu;
								$rincian_persen_kotabogor_satu = $detail->rincian_persen_kotabogor_satu;
								$rincian_kotabogor_dua = $detail->rincian_kotabogor_dua;
								$rincian_persen_kotabogor_dua = $detail->rincian_persen_kotabogor_dua;
              ...
              <figure class="highcharts-figure">
                ...
                <ol>
                  <li>Hasil jumlah perolehan suara sah pasangan calon 1 yaitu <?=$detail->nama_capres_satu?> tahun
                    <?=$detail->tahun?>
                    sebanyak <?= number_format((float)$detail->rincian_kotabogor_satu)?> atau sekitar
                    <?php
                 $a= $detail->rincian_kotabogor_satu;
								 $b= $detail->rincian_kotabogor_dua;
                 $hasil = $a / ($a + $b)*100;
									?>
                  ...
                  <li>Hasil jumlah perolehan suara sah pasangan calon 2 yaitu <?=$detail->nama_capres_dua?> tahun
                    <?=$detail->tahun?>
                    sebanyak <?= number_format((float)$detail->rincian_kotabogor_dua)?> atau sekitar
                    <?php
                 $a= $detail->rincian_kotabogor_dua;
								 $b= $detail->rincian_kotabogor_satu;
                 $hasil = $a / ($a + $b)*100;
									?>
                    <?= number_format((float)$hasil, 1)?>%
                  </li>
