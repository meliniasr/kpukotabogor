        ...
        <div class="card">
          ...
          <div class="card-body">
            <h4 style="font-family:'Franklin Gothic Medium', 'Arial Narrow', Arial, sans-serif; text-align:center;">
              PEROLEHAN SUARA SAH PASANGAN CALON PRESIDEN DAN WAKIL PRESIDEN <br> TAHUN <?= $detail->tahun?> DI PROVINSI
              JAWA
              BARAT
            </h4>
            <br>
                        ...
                        <div class="card-body">
                          ...
                          <table id="example1" class="table table-bordered table-striped">
                            ...
                            <tbody>
                              <?php
                      $no = 1;
                      foreach ($piecharprovinsi as $detail) { ?>
                              <tr id="tabel">
                                <td><?= $no++ ?></td>
                                <td colspan="2"><?= $detail->nama_capres_satu?></td>
                                <td><?= number_format((float)$detail->rincian_provinsi_satu)?></td>
                                <td><?= number_format((float)$detail->rincian_persen_provinsi_satu, 1)?>%</td>
                                <td rowspan="3">
                                  <a href="<?= base_url('admin/perolehan_suarasah_provinsi/edit/' . $detail->id) ?>"
                                    class="btn btn-primary btn-sm"><i class="fa fa-edit"></i></a>
                                  <a onclick="return confirm('Anda yakin ingin menghapusnya?');"
                                    href="<?= base_url('admin/perolehan_suarasah_provinsi/hapus/' . $detail->id) ?>"
                              ...
                              <tr>
                                ...
                                <td><?= number_format((float)$detail->rincian_provinsi_dua)?></td>
                                <td><?= number_format((float)$detail->rincian_persen_provinsi_dua, 1)?>%</td>
                              </tr>
                              <?php } ?>
                            </tbody>
                          </table>
            ...
            Highcharts.chart('piechart_provinsi', {
              ...
              series: [{
                ...
                data: [
                  <?php
					if(is_array($piechartprovinsi)){
						foreach ($piechartprovinsi as $key => $detail) {
							  $nama_capres_satu = $detail->nama_capres_satu;
								$nama_capres_dua = $detail->nama_capres_dua;
								$rincian_provinsi_satu = $detail->rincian_provinsi_satu;
								$rincian_persen_provinsi_satu = $detail->rincian_persen_provinsi_satu;
								$rincian_provinsi_dua = $detail->rincian_provinsi_dua;
								$rincian_persen_provinsi_dua = $detail->rincian_persen_provinsi_dua;
            ...
            <div class="figure" style="color:black;">
              <figure class="highcharts-figure">
                ...
                <p class="highcharts-description">
                  ...
                  <?php foreach($piechartprovinsi as $detail) {?>
                <ol>
                  <li>Hasil jumlah perolehan suara sah pasangan calon 1 yaitu <?=$detail->nama_capres_satu?> tahun
                    <?=$detail->tahun?>
                    sebanyak <?= number_format((float)$detail->rincian_provinsi_satu)?> atau sekitar
                    <?php
                 $a= $detail->rincian_provinsi_satu;
								 $b= $detail->rincian_provinsi_dua;
                 $hasil = $a / ($a + $b)*100;
									?>
                  ...
                  <li>Hasil jumlah perolehan suara sah pasangan calon 2 yaitu <?=$detail->nama_capres_dua?> tahun
                    <?=$detail->tahun?>
                    sebanyak <?= number_format((float)$detail->rincian_provinsi_dua)?> atau sekitar
                    <?php
                 $a= $detail->rincian_provinsi_dua;
								 $b= $detail->rincian_provinsi_satu;
                 $hasil = $a / ($a + $b)*100;
									?>
                    <?= number_format((float)$hasil, 1)?>%
                  </li>
